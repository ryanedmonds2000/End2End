...
def experiment_crossfold(X, Xtest, Y, Ytest, unexplorable = [], valratio = 1, inputrationality = None, imputecopies = 50, nuisance_epochs = 50, nuisance_lam = 1, dfl_epochs = 50, pgsigma = 0.5, dataseed=42, pathseed=42, folds=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(trainset)))):
        ...
        Ypred_subset = torch.stack([
            thetadr_uber(*val_subset[i], nuisance, gram, reg=True)
...
def main():
    ...
    print(f"R={rat},")
    predmodel, trainregret, testregret, rationality = experiment_crossfold(X, Xtest, Y, Ytest, [], inputrationality=rat, dataseed=seed, nuisance_lam=0, folds=2)



    with open(f"ResultsUberConst/regret_reg_r{rat}.json", "w") as f:
        json.dump(testregret, f)

    print(f"Saved to ResultsUberConst/regret_reg_r{rat}.json")
