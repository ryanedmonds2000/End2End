...
def experiment_crossfold(X, Xtest, Y, Ytest, unexplorable, gamma = 0, inputrationality = None, imputecopies = 50, nuisance_epochs = 300, nuisance_lam = 1, dfl_epochs = 50, pgsigma = 0.5, dataseed=42, pathseed=42, folds=5):
    ...
    unexplored = dataset.get_unexplored()
    print("Imputing Data...")
    mean = dataset.y.mean() * 8
    imputeset = GammaImputeDataset(X, Y, unexplored, mean, gamma=gamma, copies=imputecopies, seed=dataseed) # Constant cost imputed data
...
def main():
    ...
    gammas = list(np.linspace(-1, 1, 11))
    ...
    for gamma in gammas:
        for r in inputrationalities:
            ...
            predmodel, trainregret, testregret, rationality = experiment_crossfold(X, Xtest, Y, Ytest, [], gamma=gamma, inputrationality=r, dataseed=42, nuisance_lam=0.5, folds=2, dfl_epochs=25)
            regrets.append(testregret)
            print(f"R={r}, Gamma={gamma}, Regret={testregret}")

    n_cols = len(inputrationalities)

    with open(f"Results/dist_regrets_table_{seed}.csv", "w", newline="") as f:
        writer = csv.writer(f)
        ...
        for row_idx, valratio in enumerate(gammas):
            row_values = regrets[row_idx * n_cols : (row_idx + 1) * n_cols]
            writer.writerow([valratio] + row_values)

    print(f"Saved to dist_regrets_table_{seed}.csv")
