...
def experiment_crossfold(X, Xtest, Y, Ytest, unexplorable, valratio = 1, inputrationality = None, imputecopies = 50, nuisance_epochs = 300, nuisance_lam = 1, dfl_epochs = 50, pgsigma = 0.5, dataseed=42, pathseed=42, folds=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(trainset)))):
        ...
        Ypred_subset = torch.stack([
            thetadr(*val_subset[i], nuisance, gram, reg=True)
...
def main():
    ...
    for r in inputrationalities:
        print(f"R={r}...")
        predmodel, trainregret, testregret, rationality = experiment_crossfold(X, Xtest, Y, Ytest, [], inputrationality=r, dataseed=42, nuisance_lam=0, folds=2)
        regrets.append(testregret)
        print(f"R={r} Regret={testregret}")

    n_cols = len(inputrationalities)

    with open(f"Results/reg_regrets_table_{seed}.csv", "w", newline="") as f:
        writer = csv.writer(f)

        # Header row: blank corner + rationality column labels
        writer.writerow(["rationality"] + inputrationalities)

        # Data rows: one
        row_values = regrets
        writer.writerow([""] + row_values)

    print(f"Saved to reg_regrets_table_{seed}.csv")
